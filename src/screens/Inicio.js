...
const Inicio = () => {
    ...
    const onShareQuote = async (nome, texto) => {
        await Share.share({
            message: texto + '\n' + '\n' + nome + '\n' + 'https://play.google.com/store/apps/details?id=com.sabio'
        });
    };
    ...
    return (
        ...
        <SafeAreaView style={styles.container}>
            {spinner ? (
                ...
                <FlashList
                    initialScrollIndex={notificationId ? Number(notificationId) : 0}
                    estimatedFirstItemOffset={notificationId ? verticalScale(20) : null}
                    data={item}
                    ...
                    renderItem={({ index, item }) => (
                        <View style={{ padding: moderateScale(12), backgroundColor: theme == false ? '#e9e9e9' : '#191919' }}>
                            <Card
                                mode={notificationId && notificationId == item.id ? 'outlined' : 'contained'}
